...
class ManageUsersController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        $widgetCount['total_deposit']      = Deposit::where('user_id',$user->id)->where('status',Status::PAYMENT_SUCCESS)->sum('amount');
        $widgetCount['total_withdraw']     = Withdrawal::where('user_id',$user->id)->where('status',Status::PAYMENT_SUCCESS)->sum('amount');
        $widgetCount['total_transactions'] = Transaction::where('user_id',$user->id)->count();

        $widgetCount['total_job']             = Job::where('user_id',$user->id)->count();
        $widgetCount['total_software']        = Software::where('user_id',$user->id)->count();
        $widgetCount['total_service']         = Service::where('user_id',$user->id)->count();
        $widgetCount['total_service_booking'] = Booking::where('buyer_id',$user->id)->count();

        return view('admin.users.detail', compact('pageTitle', 'user','totalDeposit','totalWithdrawals','totalTransaction','countries','widgetCount'));
    }
