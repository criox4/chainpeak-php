...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $widget['totalPendingServiceCount']    = Service::pending()->count();
        $widget['totalPendingSoftwareCount']   = Software::pending()->count();
        $widget['totalPendingJobCount']        = Job::pending()->count();
        $widget['totalCategoryCount']          = Category::count();
        $widget['totalSubcategoryCount']       = SubCategory::count();
        $widget['totalFeatureCount']           = Feature::count();
        $widget['totalAdCount']                = Advertisement::count();
        $widget['totalCouponCount']            = Coupon::count();
        $widget['reportedServiceBookingCount'] = Booking::where('service_id', '!=', 0)->disputed()->count();
        $widget['reportedJobHiringCount']      = JobBid::disputed()->count();
        $widget['totalSoftwareSaleCount']      = Booking::where('software_id', '!=', 0)->where('status', Status::BOOKING_PAID)->count();
        $widget['totalLevelCount']             = Level::count();

        $service = Service::query();
        $widget['total_service']        = $service->count();
        $widget['total_active_service'] = $service->approved()->count();
        $widget['total_active_service'] = $service->approved()->count();
    ...
    public function chatStore(Request $request)
    {
        ...
        if ($request->type == 'service') {
            $data      = Booking::where('id', decrypt($request->id))->where('service_id', '!=', 0)->where('working_status', Status::WORKING_DISPUTED)->firstOrFail();
            $bookingId = $data->id;

        } else {
            $data     = JobBid::where('id', decrypt($request->id))->where('working_status', Status::WORKING_DISPUTED)->firstOrFail();
            $jobBidId = $data->id;
        }
