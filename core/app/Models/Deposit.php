...
class Deposit extends Model
{
    ...
    public function scopePaymentPendingService($query)
    {
        return $query->where('order_number', '!=', null)->where('booking_id', '!=', 0)->whereHas('booking', function ($q) {
                    $q->where('service_id', '!=', 0)->where('status', Status::BOOKING_UNPAID);
                });
    }

    public function scopePaymentPendingSoftware($query)
    {
        return $query->where('order_number', '!=', null)->where('booking_id', '!=', 0)->whereHas('booking', function ($q) {
                    $q->where('software_id', '!=', 0)->where('status', Status::BOOKING_UNPAID);
                });
    }
}
